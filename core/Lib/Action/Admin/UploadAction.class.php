...
class UploadAction extends AdminAction{
	...
	public function upload(){
        ...
        if (!$up->upload()) {
			...
			exit($error.' [<a href="?s=Admin/Upload/Show/fileback/'.$fileback.'/mid/'.$mid.'">重新上传</a>]');
			//dump($up->getErrorMsg());
		}
		...
		echo '文件<a href="'.$uppath.$uploadList[0]['savename'].'" target="_blank"><font color=red>'.$uploadList[0]['savename'].'</font></a>上传成功　[<a href="?s=Admin/Upload/Show/fileback/'.$fileback.'/mid/'.$mid.'">重新上传</a>]';
		echo '</div>';
	}
	// 本地附件展示
