...
class VideoAction extends AdminAction{
	 ...
	 public function _initialize(){
		...
		$this->AddoncDB =D('Admin.Addon_class');
    }
	// 视频列表		
    public function show(){
		...
		if ($status || $status==='0') {
			$where['status'] = array('elt',intval($status));
		}
		if ($serial) {
		...
		if ($downurl=='1') {
			$where['downurl'] = array('neq','');
		}
		if ($picurl) {
			$where['Left(picurl,7)'] = array('eq','fail://');
		}		
		if ($keyword) {
			$search['id']   = array('eq',$keyword);
		...
		$video_url   = U('Admin-Video/Show',array('cid'=>$cid,'status'=>$status,'downurl'=>$downurl,'serial'=>$serial,'picurl'=>$picurl,'type'=>$video['type'],'order'=>$video['order'],'keyword'=>urlencode($keyword),'p'=>''),false,false);
		...
		foreach($list as $key=>$val){
			...
			$list[$key]['lastdowurl'] = get_downurl_last_name($list[$key]['downurl']);
		}
    ...
    public function add(){
		...
		$this->assign($array);
		$addonclass=$this->AddoncDB->order('oid asc')->select();
		$this->assign('list_channel_video',F('_gxcms/channelvideo'));
		$this->assign('list_colnode',F('_gxcms/ColNode'));
		$this->assign('list_addonclass',$addonclass);
		$this->display('views/admin/video_add.html');
    }
	// 前置操作
	public function _before_insert(){
		if ($_POST['cid2']){
			$_POST['cid2'] = implode(',',$_POST['cid2']);
		}else{
			$_POST['cid2'] = 0;
		}// 副分类数据组合 by 青春浪子
		if ($_POST['cid3']){
			$_POST['cid3'] = implode(',',$_POST['cid3']);
		}else{
			$_POST['cid3'] = 0;
		}// 扩展分类数据组合 by 青春浪子
    ...
    public function codown(){
		...
		$url = "http://www.xso.com/api/down.php?w=".$keyword;
		$location = file_get_contents($url);
		//preg_match_all("/http:\/\/www\..*? /",$location,$array);
		preg_match_all("/\?id=.*? /",$location,$idarray);
		preg_match_all("/td44\"\>[\s\S]+?\</i",$location,$datearray);
		if(count($idarray)==0){
			echo "<tr><td>暂无<td></tr>";
		}else{
			preg_match_all("/title=[\s\S]+?\"/i",$location,$titarray);
			foreach ($idarray[0] as $k => $r) {
				$array[] = array($idarray[0][$k],$titarray[0][$k],$datearray[0][$k]);
			}
			//print_r($array);
    ...
    public function showcodown(){
		...
		$add = intval($_GET['add']);
		$key = intval($_GET['key']);
		$downurl = $this->VideoDB->where('id = '.$id)->getField('downurl');
		if ($add == 1) {//+
			$weburl = "http://www.xso.com/show.php?id=".$key;
			$code = file_get_contents($weburl);
			preg_match_all("/(value=\'xzurl|value=\'cid)=.*\'/isU",$code,$urlarray);
			foreach ($urlarray[0] as $k => $r) {
				$mc = jiequstr($urlarray[0][$k],'&mc=','\'');
				$urls = $mc."$".jiequstr($urlarray[0][$k],'value=\'','&mc')."&mc=".C('down_bf').$mc."\n";				
				$data['downurl'] .= $urls;
			}
			$data['downurl'] = substr($data['downurl'],0,strlen($data['downurl'])-1);
			$data['dreurl'] = $key;
			$this->VideoDB->where('id = '.$id)->save($data);
			redirect(C('cms_admin').'?s=Admin/Video/Showcodown/id/'.$id,0);
		}elseif ($add == 2) {//-
			$data['downurl'] = '';
			$data['dreurl'] = '';
			$this->VideoDB->where('id = '.$id)->save($data);
		...
		$list = $this->VideoDB->field('id,title')->where($where)->order('id desc')->select();
		foreach($list as $key=>$value)
		{
		  $array[$key]=$value['id']."|http://www.xso.com/api/down.php?w=".$value['title'];
		}
		$title = $list['title'];
		$downurl = str_replace("\n","<br>",$downurl);
		$this->assign($array);
