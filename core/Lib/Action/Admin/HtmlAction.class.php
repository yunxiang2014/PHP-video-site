...
class HtmlAction extends AdminAction{	
    ...
    public function Allvideo($id){
		...
		$this->assign('model','allvideo');
		$this->buildHtml('allvideo','./'.C('url_dir_maps').'/','./template/'.C('default_theme').'/Home/allvideo.html');
		if (empty($id)) {
			$this->assign("jumpUrl",'?s=Admin/Html/Show/');
        	$this->success('全部影片列表生成成功！');
		}		    
    }
	//生成最新100个影片列表
    public function Newvideo($id){
		...
		$this->assign('model','newvideo');
		$this->buildHtml('newvideo','./'.C('url_dir_maps').'/','./template/'.C('default_theme').'/Home/newvideo.html');
		if (empty($id)) {
			$this->assign("jumpUrl",'?s=Admin/Html/Show/');
        	$this->success('最新100个影片列表生成成功！');
		}		    
    }
	//生成新片上映列表
    public function Releasevideo($id){
		$this->assign('webtitle','新片上映'.'-'.C('web_name'));
		$this->assign('navtitle','<a href="'.C('web_path').'">首页</a> &gt; <span>新片上映</span>');
		$this->assign('model','releasevideo');
		$this->buildHtml('releasevideo','./'.C('url_dir_maps').'/','./template/'.C('default_theme').'/Home/releasevideo.html');
		if (empty($id)) {
			$this->assign("jumpUrl",'?s=Admin/Html/Show/');
        	$this->success('新片上映列表生成成功！');
		}		    
    }
	//生成影视列表
