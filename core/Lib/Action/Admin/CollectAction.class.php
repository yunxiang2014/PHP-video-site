	...
	public function xmlmdb($xml){
		...
		if($array_url['action']){
			...
			foreach($list_vod as $key=>$vod){
				...
				$vod['hits']    = mt_rand(30,C('web_admin_hits'));
				$vod['score']   = mt_rand(6,C('web_admin_score'));
				$vod['scoreer'] = mt_rand(6,C('web_admin_score'));
				$vod['up']      = mt_rand(5,C('web_admin_updown'));
				$vod['down']    = mt_rand(5,C('web_admin_updown'));
