...
class CommentAction extends HomeAction{
    ...
    public function lists(){
		...
		$order = !empty($_GET['order'])?trim($_GET['order'].' desc'):'addtime desc';
		...
		//$where[C('db_prefix').'comment.cid'] = array('in',trim($cid) );
		...
		//分页链接参数
		//$pageurl='javascript:void(0)" onclick="CommentShow(\''.U('comment/lists',array('mid'=>$mid,'did'=>$did,'cid'=>$cid,'p'=>'{!page!}'),false,false).'\')';
