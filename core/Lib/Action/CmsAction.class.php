...
class CmsAction extends Action{
    ...
    public function create_channel(){
		...
		$list[999]['video']  = get_tpl_limit('<gxsearchajax(.*)limit="([0-9]+)"(.*)>','video_searchajax');//缓存搜索自动完成分页数据
	...
	* @返回赋值后的style数组,可在调用的类直接assign*/
    public function tags_style(){
		...
		if(C('url_html')){
			...
			$style['allvideo'] = C('web_path').$mapdir.'allvideo'.C('html_file_suffix');
			$style['newvideo'] = C('web_path').$mapdir.'newvideo'.C('html_file_suffix');
			$style['releasevideo'] = C('web_path').$mapdir.'releasevideo'.C('html_file_suffix');
			...
			$style['newvideo']  = get_show_url('newvideo','',1);
			$style['allvideo']  = get_show_url('allvideo','',1);
			$style['releasevideo']  = get_show_url('releasevideo','',1);
		}		
		return $style;
    }	
	/*****************影视内容,播放页变量解析******************************
	...
	* @返回赋值后的arrays 多维数组*/
	public function tags_video_read($array,$playarr=false){
		...
		$array['downlist']      = $this->downlist($array['downurl'],$array['id'],$array['cid']);
		$array['count']         = $array['playlist'][0]['playcount'];
		$array['content']       = preg_replace("'<a[^>]*?>.*?</a>'si",'',$array['content']);
		if (C('web_admin_yswyc')) {
		$array['content']       = strtr($array['content'],$keyword_arr);//影片介绍伪原创
		}
		$array['playurl_last'] = get_play_url($array['id'],$array['cid'],$array['count']);
		...
		if($playarr){
			...
			if (C('user_pay')  && in_array($array['cid'],C('user_paycid'))){
				...
				//$player = $array['playurl'];
				//https://www.youtube.com/v/8npJW5JNLYk
				//echo $array['playurl'];
				$var = $array['playurl'];
				//echo $var;
				//$player .='<embed height="480" src="'.$var.'" type="application/x-shockwave-flash" width="853"></embed>';
$player .='<iframe width="853" height="480" src="'.$var.'" frameborder="0" allowfullscreen></iframe>';
	...
	//分解下载地址链接
	public function downlist($vodurl,$id,$cid){
		if(!$vodurl){
			return array();
		}
		...
		foreach($downlist as $key=>$val){
		    if (strpos($val,'$') > 0) {
		        $ji = explode('$',$val);
			    $list['downname'] = trim($ji[0]);
			    $list['downpath'] = trim($ji[1]);
			}else{
			    $list['downname'] = '第'.get_play_name($key+1,$count).'集';
			    $list['downpath'] = trim($val);
			}
			$list['downcount'] = count($downlist);
		    $urllist[]         = $list;
		}
	    return $urllist;
	}
	...
	* @返回赋值后的arrays 多维数组*/
	public function tags_info_read($array){
		...
		if (C('web_admin_zxwyc')) {
		$array['content']     = strtr($array['content'],$keyword_arr);//资讯内容伪原创
		}
